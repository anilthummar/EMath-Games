              ...
              children: <Widget>[
                GestureDetector(
                    ...
                    child: CustomTextLabelWidget(
                        label: context.appStrings.detailPageWithBottomBarKey)),
                const SizedBox(
                  height: Dimens.size50,
                ),
                CustomTextLabelWidget(label: context.appStrings.appNameKey),
                const SizedBox(
                  height: Dimens.size12,
                ),
                CustomTextLabelWidget(
                    label:
                        "\\${context.appStrings.emailKey} :- \\${state.userDetailResponse?.data?.email} \\n \\${context.appStrings.nameKey} :- \\${state.userDetailResponse?.data?.firstName} \\${state.userDetailResponse?.data?.lastName}"),
              ]),
        );
      },
    );
