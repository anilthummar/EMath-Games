  ...
  Future<ResponseHandler<T?>> handleApiCall<T>(String endUrl, ApiType apiType,
    ...
    try {
      ...
      handler = OnFailureResponse<T?>(
        error: ErrorResult(
          errorMessage: MainConfig.context.appStrings.badRequestStateKey,
  ...
  ResponseHandler<T?> _responseHandler<T>(Response<T>? response) {
    if (response?.statusCode == 200) {
      ...
      return OnFailureResponse<T?>(
        error: ErrorResult(
          errorMessage: MainConfig.context.appStrings.unauthorizedKey,
          type: DioExceptionType.unknown,
        ),
        statusCode: 401,
      );
      ...
      return OnFailureResponse<T?>(
        error: ErrorResult(
          errorMessage: MainConfig.context.appStrings.serverNotRespondKey,
      ...
      return OnFailureResponse<T?>(
        error: ErrorResult(
          errorMessage: MainConfig.context.appStrings.somethingWentWrongKey,
          type: DioExceptionType.unknown,
        ),
      );
    }
  ...
  Future<void> _checkInternetConnection(
    ...
    if (isInternetDialogVisible) {
      ...
      return handler.reject(DioException(
        ...
        error: MainConfig.context.appStrings.noInternetConnectionKey,
      ));
    } else {
      if (isConnected) {
        ...
        if (!isInternetDialogVisible) {
          ...
          await showCustomDialog(
            MainConfig.context.appStrings.noInternetConnectionDescriptionKey,
            title: MainConfig.context.appStrings.noInternetConnectionKey,
            okBtnTitle: MainConfig.context.appStrings.retryKey,
            ...
            cancelBtnTitle: MainConfig.context.appStrings.cancelKey,
            onCancelClicked: () {
              isInternetDialogVisible = false;
              //reject current api call
